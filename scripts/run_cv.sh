...
if [[ "$SKIP_CV" == "1" ]]; then
    ...
    if (( _cv_total == 0 )); then
        ...
        _do_cv() {
            ...
            LR=$LR WARMUP_FRACTION=$WARMUP_FRACTION GRAD_CLIP=$GRAD_CLIP \
            BATCH=$BATCH EXTRACT_BATCH=$EXTRACT_BATCH N_GPUS=1 \
            INPUT_DIR=$INPUT_DIR PKL_DIR=$_pkl PT_DIR=$_pt \
            OUT_ROOT=$OUT_ROOT OUT_SUFFIX=$_sfx \
            CKPT=$_ck CONFIG=$_cf INIT_FROM_THERMO=$_init \
            HEAD_HIDDEN=$HEAD_HIDDEN N_MP_LAYERS=$N_MP_LAYERS MP_N_HEADS=$MP_N_HEADS \
...
for rep in sorted(glob.glob(os.path.join(root, "*/cv_report.json"))):
    ...
    try:
        ...
        rows.append((suffix, math.nan, math.nan, math.nan, math.nan, math.nan, math.nan, 0))

if not rows:
    print("No cv_report.json found under", root)
else:
    # Group columns by metric block: best-val (MAE, RMSE, R²) and last-stable (same).
